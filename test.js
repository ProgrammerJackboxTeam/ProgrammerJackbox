...
console.log('   Non-deciders:', logiccah.getNonDeciders().map(p => p.name).join(', '));
...
console.log(`   Correctly identified: ${results.correctlyIdentified.map(id => testPlayers.find(p => p.id === id).name).join(', ')}`);
console.log(`   Not identified: ${results.notIdentified.map(id => testPlayers.find(p => p.id === id).name).join(', ')}`);
